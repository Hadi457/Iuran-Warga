...
<div class="container py-4">
    ...
    <div class="table-responsive-md">
        ...
        <tbody>
            ...
            <td>{{ Str::limit($item->addres, 20) }}</td>
            ...
            <td class="text-center">
                <a class="btn btn-sm btn-warning" href="{{route('warga-edit', Crypt::encrypt($item->id))}}" title="Edit Warga">
                <i class="fa-solid fa-pen-to-square"></i>
                </a>
                <a class="btn btn-sm btn-danger" href="{{route('warga-delete',Crypt::encrypt($item->id))}}" onclick="return confirm('Hapus data ini?')" title="Hapus Warga">
    ...
    <nav aria-label="Page navigation">
        ...
        @else
            <li class="page-item"><a class="page-link" href="{{ $warga->previousPageUrl() }}">Previous</a></li>
        @endif

        @foreach ($warga->getUrlRange(1, $warga->lastPage()) as $page => $url)
            <li class="page-item {{ $warga->currentPage() == $page ? 'active' : '' }}">
            <a class="page-link" href="{{ $url }}">{{ $page }}</a>
            </li>
        @endforeach

        @if ($warga->hasMorePages())
            <li class="page-item"><a class="page-link" href="{{ $warga->nextPageUrl() }}">Next</a></li>
        @else
